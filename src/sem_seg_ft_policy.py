...
task_cat2mpcat40 = [
    3,  # ('chair', 2, 0)
    10,  # ('couch', 'sofa', 9, 5),
    14,  # ('potted plant', 'plant', 13, 8)
    11,  # ('bed', 'bed', 10, 6)
    18,  # ('toilet', 17, 10),
    22,  # ('tv', 'tv_monitor', 21, 13)
    5,  # ('dining-table', 'table', 4, 1)
    40, # (oven, 'misc')
    15,  # ('sink', 14, 9)
    40, # (refrigerator, 'misc')
    40, # (book, 'misc')
    40, # (clock, 'misc')
    40, # (vase, 'misc')
    40, # (cup, 'misc')
...
class SemSegSeqNet(Net):
    ...
    def _extract_sge(self, observations):
        ...
        if "semantic" in observations and "objectgoal" in observations:
            ...
            goal_visible_pixels = (obj_semantic == idx.unsqueeze(1)).sum(dim=1) # Sum over all since we're not batched
...
class SemSegSeqFTModel(nn.Module):
    def __init__(
        self,
        observation_space: Space,
        action_space: Space,
