...
from habitat_sim.utils.common import quat_to_coeffs, quat_from_coeffs
...
class ObjectGoal_Env(habitat.RLEnv):
    ...
    def load_replay_episode(self):
        ...
        pos = episode["start_position"]
        # rot = quaternion.from_float_array(episode["start_rotation"])
        rot = quat_from_coeffs(episode["start_rotation"])

        goal_name = episode["object_category"]
        goal_idx = coco_categories[episode["object_category"]]
        floor_idx = 0
        ...
        obs = self._env.sim.get_observations_at(pos, rot)
        self._current_episode_new = episode

        return obs

    def get_goal_positions(self, goal_category_id):
        semantic_scene = self._env.sim.semantic_annotations()
        goals = []
        for obj in semantic_scene.objects:
            if obj is None:
                continue
            category_name = obj.category.name()
            if coco_categories.get(category_name) == goal_category_id:
                goals.append({"position": obj.aabb.center.tolist()})
        return goals
    ...
    def generate_new_episode(self):
        ...
        self._current_episode_new = {
            "episode_id": self.episode_no,
            "scene_id": self.scene_name,
            "start_position": pos,
            "start_rotation": quat_to_coeffs(rot).tolist(),
            "object_category": self.goal_name,
            "object_id": int(self.goal_idx),
            "floor_id": int(floor_idx),
            "goals": goals,
        }
    ...
    def reset(self):
        ...
        obs["gps"] = [self.last_sim_location[0], -self.last_sim_location[1]]
        obs["compass"] = self.last_sim_location[2]
